...
******************************************************************************/
#ifndef MODELS_INCLUDE_TORCAAT_HPP_
#define MODELS_INCLUDE_TORCAAT_HPP_
...
enum OAT_OP{
    ...
    SUB, RSB, RSC,  // subtraction, reverse subtraction and subtraction w/vcarry
    ...
    // saturation
    SSAT, SSAT16,  // saturate signed 32-bit value, saturate two signed 16-bit
    ...
    ADD16,  // add two half
    // signed add two half, add two half saturating, add two half halving
    SADD16, QADD16, SHADD16,
    ...
    MOVT,  // move top
    RBTI,  // reverse bits
    REV, REV16, REVSH,  // byte-reverse word, packet halfword, and signed hword
    SBFX, UBFX,  // signed and unsigned bit field extract
    SEL,  // select bytes (uses GE flags)
    USAD8, USADA8,  // unsigned sum of absolute diffences and its accumulative
    ...
    STRH, STRHT, STREXH,  // store 16-bit halfword, unprivileged, and exclus.
    LDRH, LDRHT, LDREXH,  // load 16-bit unsigned
...
class TOrcaAT : TProcessorBase<uint32_t>{
 ...
 public:
    void Reset();
    explicit TOrcaAT(std::string name);
    ~TOrcaAT();
    SimulationTime Run();
};
#endif  // MODELS_INCLUDE_TORCAAT_HPP_
