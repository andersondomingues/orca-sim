...
TNetSocket::TNetSocket(std::string name) : TimedModel(name) {
	...
	//_udp_server = new udp_server(server_addr, 8888);
...
void TNetSocket::udpToNocProcess(){
	...
	this->output_debug << " intr:" << _comm_intr->Read() 
			  << " start:" << _comm_start->Read()
			  << " ack:" << _comm_ack->Read() << std::endl;
...
void TNetSocket::nocToUdpProcess(){	
	...
	if(_comm_intr->Read() != 0){ //has packets 
		...
		int8_t* msg = new int8_t[64]; //64 flits = 1 msg
		_mem1->Read(_mem1->GetBase(), msg, 64);

		_udp_client->send((const char*)msg, 64);
