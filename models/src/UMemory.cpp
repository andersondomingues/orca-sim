...
void UMemory::Write(MemoryAddr addr, MemoryType* data, uint32_t length){
	...
	//@TODO: check whether this has the best performance
...
void UMemory::LoadBin(std::string filename, uint32_t base, uint32_t size){
	...
	if(f.is_open()){
		f.read((char*)&_mem[base -_base], sizeof(_mem[0]) * size);
		f.close();
...
void UMemory::SaveBin(std::string filename, uint32_t base, uint32_t size){
	...
	if(f.is_open()){
		f.write((char*)&_mem[base -_base], sizeof(_mem[0]) * size);
		f.close();
	}else{
...
void UMemory::Dump(uint32_t base, uint32_t length){
	...
	for(k = 0; k < length; k += 16){
		...
		//print the data in ascii ("." is printed when no ascii char exists for that byte)
		printf(" |");
		for(l = 0; l < 16; l++){
...
[[deprecated("This method has no meaning in a stateless model such as UMemory and will be removed soon.")]]
void UMemory::Reset(){}
